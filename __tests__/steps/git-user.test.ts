...
test('throws organization', async () => {
  await expect(gitUser.findGitUser('fixrtm', octokit)).rejects.toThrow(
    "You can't commit as a Organization"
  )
})

test('user account', async () => {
  await expect(gitUser.findGitUser('anatawa12', octokit)).resolves.toEqual([
    'anatawa12',
...
test('github actions account', async () => {
  await expect(
    gitUser.findGitUser('github-actions[bot]', octokit)
  ).resolves.toEqual([
    'github-actions[bot]',
